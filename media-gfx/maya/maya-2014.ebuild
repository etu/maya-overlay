...
RDEPEND="${RDEPEND}
	=media-gfx/adlmapps-7
	=media-gfx/adlmflexnet-7
"
...
src_unpack() {
	...
	rpm2tar Maya2014_64-2014.0-904.x86_64.rpm -O | tar -x
	assert "Failed to unpack Maya2014_64-2014.0-904.x86_64.rpm"
}
...
src_install() {
	...
	mkdir -p ${D}usr/bin/
	ln -s /usr/autodesk/maya2014-x64/bin/maya2012 ${D}usr/bin/maya
	ln -s /usr/autodesk/maya2014-x64/bin/Render   ${D}usr/bin/Render
	ln -s /usr/autodesk/maya2014-x64/bin/fcheck   ${D}usr/bin/fcheck
	ln -s /usr/autodesk/maya2014-x64/bin/imgcvt   ${D}usr/bin/imgcvt

	ln -s maya2014-x64 ${D}usr/autodesk/maya
...
pkg_postinst() {
	...
	einfo " # echo 'MAYA_LICENSE=unlimited'      >  /usr/autodesk/maya2014-x64/bin/License.env"
	einfo " # echo 'MAYA_LICENSE_METHOD=network' >> /usr/autodesk/maya2014-x64/bin/License.env"
	einfo
	einfo
	einfo "And then you need to run this as root to activate your license:"
	einfo " # LD_LIBRARY_PATH=/opt/Autodesk/Adlm/R4/lib64 /usr/autodesk/maya2014-x64/bin/adlmreg -i S 657D1 657D1 2012.0.0.F <your serial number> /var/opt/Autodesk/Adlm/Maya2014/MayaConfig.pit"
	einfo
	einfo
	...
	einfo " # mkdir ~/maya; cd ~/maya; cp /usr/portage/distfiles/Autodesk_Maya_2014_SP3_English_Linux_64bit.tgz .; tar -xf Autodesk_Maya_2014_SP3_English_Linux_64bit.tgz; ./setup; cd ~; rm -rf ~/maya"
